...
void ATA_PIO_wait_BSY(){
    while(io_in8(ATA_1_1F7) & PIO_STATUS_BSY);
}

void ATA_PIO_wait_DRQ(){
    while(!(io_in8(ATA_1_1F7) & PIO_STATUS_RDY));
}
...
void ATA_PIO_read_sectors(void* dst, uint32_t first_sector, uint8_t num_sectors){
	...
	io_out8(ATA_1_1F6, 0xE0 | ((first_sector >>24) & 0xF));
	io_out8(ATA_1_1F2, num_sectors);
	io_out8(ATA_1_1F3, (uint8_t) first_sector);
	io_out8(ATA_1_1F4, (uint8_t)(first_sector >> 8));
	io_out8(ATA_1_1F5, (uint8_t)(first_sector >> 16)); 
	io_out8(ATA_1_1F7, 0x20); //Send the read command
...
void ATA_PIO_write_sectors(void* src, uint32_t first_sector, uint8_t num_sectors){
	...
	io_out8(ATA_1_1F4, (uint8_t)(first_sector >> 8));
	io_out8(ATA_1_1F5, (uint8_t)(first_sector >> 16)); 
	io_out8(ATA_1_1F7, 0x30); //Send the write command
